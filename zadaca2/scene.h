...
public:
    Vec3f castRay(const Ray &ray, int n) const {
        Vec3f hitPoint;
        Vec3f hitNormal;
        Object* hitObject;
        ...
        if (sceneIntersect(ray, hitPoint, hitNormal, hitObject)) {
            ...
            if (n == 0) {
                ...
                Vec3f retColor = ((hitObject->material.diffuseColor * diff) * (1 - hitObject->material.reflCoef));

                Vec3f bounceColor = castRay(reflRay, n-1);

                retColor = retColor + bounceColor * hitObject->material.reflCoef;

                // For opacity
                Ray troughRay{hitPoint + ray.direction * 0.01, ray.direction};

                retColor = retColor * (1 - hitObject->material.opacityCoef) + castRay(troughRay, n-1) * hitObject->material.opacityCoef;

                // For specular highlight
                retColor = retColor + Vec3f(1, 1, 1) * spec;
